...
- **Meaning drift is a human judgement call.** The guardrails check tokens,
  length, structure and style — they cannot prove the rewrite means what
  the source meant. We know: washing an earlier draft of this README,
  every guardrail passed while the model inverted the lead sentence. Lists,
  tables and other structure are now excluded from washing entirely, but
  within a plain paragraph only a reader catches an inversion. Interactive
